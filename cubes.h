...
/**
 * Calculates rank for given (z_1, z_2, ..., z_k) cube.
 * @details Formula: rank(z_1,z_2,...,z_k) = (z_1-1)+(z_2-1)n_1 + (z_3-1)
 * n_1n_2 + ... + (z_k-1)n_1n_2...n_{k-1}.
...
/**
 * Calculates coordinate array for a given cube from its rank. Inverse
...
/**
 * Moves from one cube to another in one dimension by a integer steps by
 * only using their ranks and calculates rank of the destination.
 * @param cubeRank - Rank of cube.
...
/**
 * Gets maximum value of rank function for given input data.
 * @param d - Pointer to input data.
